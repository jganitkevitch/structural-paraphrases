...
application for this adaptation: introduction of rule-level features,
the choice of an objective function and a development set for
parameter estimation, and the injection of addional rules into the
paraphrase grammar. In the following, we discuss each of these in
detail on the example of adapting our paraphrasing system to perform
sentence compression. \mnote{..and reference expansion?}
...
effects. To make this infromation available to the decoder, we add the
following features:
\begin{itemize}
\item $c_{\mathit{src}}$ and $c_{\mathit{tgt}}$, which count the
  number of words on either side of the rule.
\item $c_{\mathit{diff}}$, the change in the number of words the rule
  effects.
\item $\delta_{\mathit{lex}}$, a binary indicator for whether the rule
  is purely phrasal.
\item $\delta_{\mathit{del}}$, a binary indicator for whether the rule
  deletes all terminal symbols.

\item $\delta_{\mathit{reorder}}$, which is 1 if the rule swaps the
  order of two nonterminals

\end{itemize}


objective function in parameter tuning
  and data used for tuning (contrast to old approaches, mostly
  precision on gold paraphrase set and BLEU)

Injection of appropriate rules into the grammar
