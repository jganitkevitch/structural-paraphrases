...
%  the reactor spews radiation \\
%  radiation is spewing from the reactor
\end{tabular}
...
\item A development set containing examples of the sentential transformations that we would like to model.  The dev set is used with the objective function to set the weights of the features.
\item Optionally, a way of injecting task-specific rules that were not extracted automatically.
\end{itemize} 
In the following subsections, we discuss each of these in detail on the example of
adapting our paraphrasing system to perform sentence compression,
which is the task of reduce the length of sentence while preserving its core
meaning.
