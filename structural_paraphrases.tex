...
machinery. \mnote{Should we assume familiarity with the whole
  source-target stuff? Probably not. Need to introduce.} The model we
...
To compute the weight $w$ for the extracted rules, SAMT relies on a
log-linear model:
\begin{equation}
r \in \mathcal{R}; ~~ w_r = \sum_i^N \lambda_i \varphi_i(r)
\end{equation} \mnote{This isn't very nice, formally. Rethink.}
Here, the $\varphi_i$ are a set of feature functions with
corresponding model weights $\lambda_i$.
