...
\begin{table*}[!ht]
  ...
  \begin{tabular}{|c|rrcl|}
    \hline
    Possessive rule & $\mathit{NP}$ $\rightarrow$  & the $\mathit{NNP}$'s
    $\mathit{NN}$ & $\mid$ & the $\mathit{NN}$ of the $\mathit{NNP}$
    \\
    ...
    Light verbs & $\mathit{SBAR/S}$ $\rightarrow$ & in order
    to $\mathit{VB}$ that & $\mid$ & in order to $\mathit{VB}$ \\

    Topicalization  & $\mathit{SBAR/S}$ $\rightarrow$ & in order
    to $\mathit{VB}$ that & $\mid$ & in order to $\mathit{VB}$ \\
...
along with the generic representations our system is able to extract
for them. Among our example transformations we can discern three
groups that differ in how easily we can find generalizing patterns for
them.

The first group encompasses transformations such as the
\emph{possessive rule} or the \emph{dative shift}. These typically
operate on nouns and noun phrases and do not require much lexical
evidence to determine their applicability. This renders these
transforms well-suited to be captured with our synchronous CFG-based
approach.

The second group of paraphrases 

\emph{THINK OUT ARGUMENTATION!} 
