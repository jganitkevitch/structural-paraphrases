...
\title{Text-to-Text Generation with Syntactic Paraphrases Learned from
...
\newcite{Callison-Burch2005} define a paraphrase probability to rank
them:
\begin{eqnarray}
...
arbitrary number of nonterminals.
...
\begin{table*}[!ht]
  ...
  \begin{tabular}{|c|rrcl|}
    ...
    Light verbs & $\mathit{VP}$ $\rightarrow$ & to take action &
    $\mid$ & to act \\
    \hline
...
appropriate frame of evidence. This is important for properly
representing the dative shift, which is a reordering transformation that
applies to certain di-transitive verbs while other verbs are uncommon
in one of the forms\footnote{Note how our system extracts a dative
  shift rule for \emph{to give} and a rule that both shifts and
  substitutes a more appropriate verb for the new form for \emph{to
    provide}}.

The ability to generalize from phrasal paraphrases is also a key
property of the work of \newcite{Madnani2007}. However, the syntactic
information we include in our paraphrases permits us to capture more
complex transforms and impose more stringent constraints on the
application of our paraphrases. For instance, the formalism used by
\newcite{Madnani2007} does not impose any constraints on the spans
covered by nonterminals and thus, to avoid spurious ambiguity in
partitioning a span, does not permit adjacent nonterminals. However,
for some transformations the identification of possibly adjacent
syntactic phrases is crucial. Examples of such transforms are the
\emph{adverbial phrase shift}, the \emph{reduction of a relative
  clause} and \emph{topicalization}.
...
model has no means of representing the morphological changes that the
verb undergoes. Still, for the special case when the active voice form
is already in the past participle and the verb thus does not change,
we manage to learn a rule. Another example of the neccessity of
morphological information are \emph{light verb} constructs. Here, a
noun is transformed in the corresponding verb -- something our
synchronous syntactic CFG approach is not able to capture.
...
\section{Text-to-Text Applications} \label{sentential_paraphrasing}

\begin{itemize}
\item the formalism we use makes sentential paraphrasing straighforward
\item this is good because most text-to-text tasks operate on
  sentence-level
\item the inclusion of high-level syntactic information makes our
  grammar a powerful tool to reshape sentential structures according
  to the task at hand
\item there are several points of application for the adaptation of
  our grammar to a specific task
\item rule feature functions, objective function in parameter tuning
  and data used for tuning (contrast to old approaches, mostly
  precision on gold paraphrase set and BLEU)
\item injection of appropriate rules into the grammar 
\item in the following we discuss a number of tasks and give examples
  on how to adapt the system to them.
