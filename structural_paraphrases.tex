...
informed \emph{ synchronous context-free grammar} (SCFG).  The SCFG formalism \cite{} \mnote{cite the Aho and Ullman 1970s book Parsing, Translation and Compiling} was re-popularized for statistical machine translation by \newcite{Chiang2005}.  
...
\mnote{cite phrase extraction heuristic papers, and say that they were
  extended to parsed parallel corpora, citing GHKM, SAMT} The rule set
...
of rules obtained from a sentence pair. To extract a \mnote{redo Fig 1
  as we discussed, so that it fits into one column, is for german, and
  uses lines for alignments instead of a grid.} To extract a rule, we
first choose a target span $e$. The left-hand side of the rule is then
given by the syntactic constituent governing $e$. However, in doing so
we can only assign left-hand side labels to a small portion of the
...
constituents \cite{Steedman1999}.  \mnote{Add an SAMT derivation, and
  in the text list some of the rules that were applied}.
