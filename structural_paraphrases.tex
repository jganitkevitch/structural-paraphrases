...
\section{Task-Specific System Adaptation} 
\label{adaptation}

As we demonstrate in Section~\ref{analysis}, \mnote{Swap the sections
  after all?} our model produces good quality syntactic paraphrases
and is capable of sentential transformations. However, this alone may
not be sufficient when deploying the system for a particular task such
as sentence compression (which calls for paraphrases that shorten the
text) or references expansion for machine translation tuning (where it
is of interest to diversify the reference set by producing sentential
paraphrases that differ significantly from the input).

To specialize our system to given tasks, 
In parameter estimation, we strive to tune our model to achieve best
performance 
...
\mnote{Is this more of a Related Work kind of thing?}
