...
\emph{SAMT} \cite{Zollmann2006}, which is based on \emph{probabilistic
  synchronous context-free grammar} (PSCFG) formalism and allows for
an arbitrary number of non-terminals.
...
\end{equation*}
where $X \in \mathcal{N}$ is a non-terminal, $\gamma \in (\mathcal{N}
\cup \mathcal{T}_S)^*$ and $\alpha \in (\mathcal{N} \cup
\mathcal{T}_T)^*$ are strings of terminal and non-terminal symbols
with an equal number of non-terminals and $\sim$ constitutes a
one-to-one correspondency relation between the non-terminals in
$\gamma$ and $\alpha$. A non-negative weight $w \geq 0$ is assigned to
each rule.
...
sentence-parallel corpora where one side (typically the target) is
annotated with syntactic parses. Figure~\ref{samt_extraction} shows an
example of rules obtained for one phrase pair. To extract a rule, we
first choose a target span $t$. The left-hand side of the rule is then
given by the syntactic constituent governing $t$. In addition to
single constituent non-terminals, SAMT allows for the concatenation of
constituents as well as for CCG-style non-terminal labels
\cite{Steedman1999}. The source side of the rule, $s$, is obtained by
projecting $t$ over the word alignment. To introduce non-terminals
into the rule's source and target sides, we can apply rules extracted
over sub-phrases of $t$, synchronously substituting the corresponding
non-terminal symbol for the sub-phrases on both sides. \mnote{Tie this
  back to $\gamma$ and $\alpha$?} The synchronous substitution applied
to $t$ and $s$ yields the correspondency $\sim$.

To compute the weight $w_r$ for an extracted rule $r$, SAMT relies on
a log-linear model:
\begin{equation}
  w_r = \sum_i^N \lambda_i \varphi_i(r)
\end{equation}
Here, the $\varphi_i$ are a set of feature functions with
corresponding model weights $\lambda_i$. The feature set of the SAMT
model by default consists of 23 feature functions and covers a diverse
set of rule characteristics. We discuss our task-specific extensions
to the SAMT feature set in Section~\ref{adaptation}.
