...
%Hierarchical phrases contain a variable  \newcite{Madnani2007}'s paraphrase table contains these slotted patterns as well. Because hierarchical phrase-based machine translation ,  \newcite{Madnani2007} mirror Hiero's log-linear model and its feature set. The parameters for the log-linear model are estimated using minimum error rate training, maximizing the BLEU metric on a set of parallel English sentences. The authors report significant gains in translation quality when using additional references generated by paraphrasing to tune a machine translation system.



Their system also employs a log-linear model that combines translation and lexical probabilities and is tuned to maximize precision over a hand-labeled set of paraphrases.
...
\newcite{Zollmann2006}: in addition to single constituent
...
For our experiments, we extracted a paraphrase grammar from the
French-English part of the Europarl version 5 corpus. The bitext was
aligned using the Berkeley aligner and the English side was parsed
using Scott's Parser\mnote{needs citations for corpus, aligner, parser
  and Thrax}. We obtained the inital translation grammar using the
SAMT toolkit by \newcite{Zollmann2006}. For paraphrase grammar
extraction, our pruning settings were $c_{\mathit{min}}=3$,
$p_{\mathit{min}}=10^{-4}$ and $n=25$. We also created a baseline
Hiero paraphrase grammar. The translation grammar for this was
generated using the Thrax grammar extraction toolkit, with identical
pruning settings for the paraphrase grammar extraction.
