...
\mnote{cite phrase extraction heuristic papers, and say that they were extended to parsed parallel corpora, citing GHKM, SAMT}
...
an example of rules obtained from one sentence pair.\mnote{redo Fig 1 as we discussed, so that it fits into one column, is for german, and uses lines for alignments instead of a grid.} To extract a
...
\mnote{Add an SAMT derivation, and in the text list some of the rules that were applied}.
