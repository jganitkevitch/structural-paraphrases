...
paraphrases. \mnote{Need better tie-in with the overall theme of
...
learn a rule. Another example of the neccessity of morphological
information are \emph{light verb} constructs such as
\begin{center}
\begin{tabular}{l}
  to take a \emph{walk} \\
  to \emph{walk} .\\
...
Many text-to-text generation tasks such as sentence compression or
simplification can be re-formulated as sentential paraphrasing
problems. Since in our model, we are reusing much of the machinery
known from statistical machine translation -- a sentential rewriting
problem itself -- our system is well-equipped to handle those tasks.


,
including grammars, decoders, language models and typical feature
functions. Like many text-to-text generation tasks, the machine
translation problem is defined on the sentence level.


However, there are several subtle yet important differences to
consider when moving to text-to-text generation.
 
\begin{itemize}
\item the formalism we use makes sentential paraphrasing
  straighforward
